...
<div class="w3-main w3-content w3-padding" style="max-width:1200px;margin-top:100px">
  ...
  {
	  $aad = mysqli_real_escape_string($conn, $row1['aadharnumber']);
  ...
  <?php } ?>
    <div class="w3-quarter">
      <img src='<?php echo $image ; ?>'  alt="no image" width="200" height="200">
        <h3><?php echo $row1["firstname"];?></h3>
	  <h3><?php echo $row1["aadharnumber"];?></h3>
	  <form action="dispvoter.php" method="POST">
    <input id='btn' name="submit<?php echo $count; ?>" type='submit' value='View' class="loginbtn">
	<?php 
	
	if(isset($_POST["submit".$count])){
		$_SESSION['user']=$row1["aadharnumber"];
		
	header("Location:http://localhost/voting/disuser.php");
	}
	?>
	</form>
